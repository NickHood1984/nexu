...
const askFeishuChoiceParameters = {
  type: "object",
  properties: {
    question: {
      type: "string",
      description: "要向用户提出的问题。适合确认、审批、二选一或有限选项选择。",
    },
    details: {
      type: "string",
      description: "补充说明，可选。支持 markdown。",
    },
    header_title: {
      type: "string",
      description: "卡片头部标题，默认“请做选择”。",
    },
    header_template: {
      type: "string",
      enum: ["grey", "red", "orange", "yellow", "green", "blue", "purple", "default"],
    ...
    options: {
      type: "array",
      description: "供用户点击的选项列表，建议 2 到 5 个。",
      minItems: 1,
      items: {
        type: "object",
        properties: {
...
function buildChoiceCard(params) {
  ...
  return buildCard({
    header: {
      title: {
        tag: "plain_text",
        content: params.header_title || "请做选择",
      },
      template: params.header_template || "blue",
    },
    elements,
  });
}
...
const plugin = {
  ...
  description:
    "在飞书会话中发送交互卡片给用户，优先用于确认、审批和有限选项提问，减少纯文本追问。",
  register(api) {
    api.registerTool(
      onlyInFeishuSession((ctx) => {
        ...
        return {
          name: "ask_feishu_choice",
          description: `在飞书里向用户发起一个带按钮的选择题或确认题。

当你需要用户做出明确选择、确认某个操作、在几个候选项里选一个，或者希望用户尽快点击反馈时，优先使用本工具而不是普通文本提问。
只有在用户必须自由输入长文本、补充背景、或回答开放问题时，才改用普通消息。

用户点击按钮后，会把按钮的 value 作为消息发送给机器人；系统会先立即显示“已收到，正在处理...”提示，再继续把点击结果交给机器人处理。
如果这是一次性选择流程，请记住返回的 messageId，并在后续调用 update_feishu_card，把原卡片替换为“已选择”确认态或禁用态。`,
          parameters: askFeishuChoiceParameters,
          async execute(_toolCallId, params) {
            try {
              ...
              const result = await sendCardViaController(
                agentId,
                buildChoiceCard(params),
                to,
                receiveIdType,
              );
    ...
    api.registerTool(
      onlyInFeishuSession((ctx) => {
        ...
        return {
          ...
          description: `发送飞书交互卡片给用户。适用于：你已经准备好了完整卡片结构，或需要发送比 ask_feishu_choice 更复杂的自定义卡片时使用。

如果你的目的只是向用户提一个有限选项问题、让用户确认、审批或二选一，请优先使用 ask_feishu_choice，而不是普通文本提问。

卡片支持按钮交互，用户点击按钮后会将按钮的值作为消息发送给机器人，机器人的对话上下文会包含按钮的值。
系统会先立即显示“已收到，正在处理...”提示，再继续处理后续回复。
...
如果只是向用户发起有限选项提问，优先用 ask_feishu_choice。
用于准备复杂卡片内容，然后再用 send_feishu_card 发送。
